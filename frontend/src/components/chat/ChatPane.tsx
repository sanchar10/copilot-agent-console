        ...
        <div className="flex-1 min-w-0 flex flex-col">
          <div className="relative flex-1 min-h-0">
              ...
              <div ref={scrollContainerRef} onScroll={handleScroll} className="h-full overflow-y-auto p-4">
                <div className="max-w-4xl mx-auto space-y-5">
                  {isLoadingMessages && showSpinner ? (
                    <div className="flex flex-col items-center justify-center h-full text-qd-text-muted gap-3">
            ...
            <button
              ...
              className={`absolute bottom-4 left-1/2 -translate-x-1/2 bg-qd-bg-elev text-qd-text px-3 py-1.5 rounded-full shadow-qd-md border border-qd-border text-sm flex items-center gap-1.5 hover:bg-qd-panel transition-opacity duration-200 z-10 ${showScrollButton ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
            >
    ...
    return (
      <div className="flex-1 flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center text-qd-text-muted">
            <svg
              className="w-16 h-16 mx-auto mb-4 text-qd-text-muted opacity-50"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
            ...
            <p className="text-lg font-medium text-qd-text">No session selected</p>
            <p className="text-sm mt-1 text-qd-text-dim">Select a session or create a new one to start chatting</p>
          </div>
        </div>
      </div>
    );
            ...
            {newSessionMessages.length > 0 ? (
              ...
              <div className="flex flex-col items-center justify-center h-full gap-4">
                <div className="text-center text-qd-text-muted">
                  ...
                  <p className="text-lg font-medium text-qd-text">How can I help you today?</p>
                  <p className="text-sm mt-1 text-qd-text-muted">Type a message to start a new session</p>
                </div>
                  ...
                  <div className="w-full max-w-4xl mx-auto space-y-2 px-4">
                    {newSessionStarterPrompts.map((sp, idx) => (
                      <button
                        ...
                        title={sp.prompt}
                        className="w-full text-left px-4 py-2.5 rounded-qd-md border border-qd-border bg-qd-bg-elev hover:bg-qd-panel hover:border-qd-accent-border transition-colors"
                      >
                        <div className="font-medium text-qd-text">{sp.title}</div>
                        <div className="text-sm text-qd-text-muted truncate">{sp.prompt}</div>
                      </button>
                    ))}
                  </div>
                )}
