...
export function InputBox({ sessionId }: InputBoxProps) {
  ...
  return (
    <div
      className={`border-t border-gray-200/30 bg-white/60 backdrop-blur-xl p-4 ${isDragOver ? 'ring-2 ring-blue-400 bg-blue-50/50' : ''}`}
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-end gap-2">
          ...
          <textarea
            ...
            className={`flex-1 resize-none rounded-xl border px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-transparent min-h-[48px] max-h-[200px] ${
              isDragOver ? 'border-blue-400' : isStreaming ? 'border-amber-300 bg-amber-50/80' : 'border-white/40 bg-white/50 backdrop-blur'
            }`}
            rows={1}
            disabled={isDisabled}
          />
