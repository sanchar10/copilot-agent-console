...
function AgentCard({ session, onNavigate }: { session: ActiveAgentSession; onNavigate: (id: string) => void }) {
  ...
  return (
    <div className="bg-white/70 dark:bg-[#2a2a3c]/70 backdrop-blur rounded-lg p-4 border border-white/40 dark:border-[#3a3a4e]">
      {/* Header */}
      ...
      {session.current_step && (
        <div className="mb-3 px-2 py-1 bg-white/40 dark:bg-[#1e1e2e]/40 rounded text-xs">
          <span className="text-emerald-500">▶</span>{' '}
          <span className="text-gray-700 dark:text-gray-300">{session.current_step.title}</span>
        </div>
      )}
      ...
      {/* Footer */}
      <div className="flex justify-between items-center mt-3 pt-2 border-t border-white/40 dark:border-[#3a3a4e]">
...
export function AgentMonitor({ onClose }: AgentMonitorProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/20 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white/80 dark:bg-[#252536]/95 backdrop-blur-xl border border-white/30 dark:border-[#3a3a4e] rounded-2xl shadow-2xl w-full max-w-4xl max-h-[80vh] flex flex-col">
        {/* Header */}
        <div className="flex items-center justify-between p-4 border-b border-white/30 dark:border-[#3a3a4e]">
          <div className="flex items-center gap-3">
            <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Active Agents Monitor</h2>
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {error && (
            <div className="mb-4 p-3 bg-red-50/80 dark:bg-red-900/30 border border-red-200/60 dark:border-red-800/60 rounded-lg text-red-700 dark:text-red-400 text-sm">
              {error}
            </div>
          )}
          ...
          {recentlyCompleted.length > 0 && (
            <div className="mb-4 p-3 bg-emerald-50/80 dark:bg-emerald-900/30 border border-emerald-200/60 dark:border-emerald-800/60 rounded-lg">
          ...
          {data.count === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 dark:text-gray-400">No agents currently running</p>
        ...
        {/* Footer */}
        <div className="p-3 border-t border-white/30 dark:border-[#3a3a4e] text-center text-xs text-gray-500 dark:text-gray-400">
