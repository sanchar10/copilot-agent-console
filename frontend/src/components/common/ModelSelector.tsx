...
export function ModelSelector({
  ...
  return (
    <div ref={containerRef} className="relative">
        ...
        className={isCompact
          ? `w-[120px] h-[30px] px-2.5 py-0.5 text-xs font-medium rounded-md flex items-center gap-1.5 transition-colors duration-150 ${
              !disabled
                ...
                : 'bg-qd-panel-deep dark:bg-qd-bg-elev text-qd-text-dim cursor-default border border-gray-200/60 dark:border-gray-700/60'
            }`
          : `w-full px-3 py-2 border rounded-lg text-sm flex items-center justify-between transition-colors ${
              !disabled
                ? 'border-qd-border-soft bg-qd-bg-elev dark:bg-qd-bg dark:border-gray-600 dark:text-gray-100 hover:border-blue-300 dark:hover:border-blue-600 cursor-pointer'
                : 'border-qd-border-soft bg-qd-panel dark:bg-qd-bg dark:border-gray-600 text-qd-text-muted cursor-default'
            }`
        }
      ...
      {/* Dropdown */}
      {open && (
        <div className={`absolute top-full mt-1 bg-qd-bg-elev border dark:border-qd-border rounded-md shadow-lg dark:shadow-black/20 z-50 max-h-60 overflow-y-auto ${
            ...
            return (
              <div key={m.id}>
                <button
                  ...
                  className={`w-full text-left px-3 py-2 text-sm flex items-center justify-between ${
                    readOnly
                      ...
                      : 'hover:bg-qd-panel-deep dark:hover:bg-qd-panel'
                  } ${
                    isSelected ? 'bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400' : 'text-qd-text-dim'
                  }`}
                >
                  <span>{m.name}</span>
                  {supportsReasoning && (
                ...
                {supportsReasoning && isExpanded && (
                  <div className="px-3 py-1.5 bg-qd-panel dark:bg-qd-bg flex gap-1 flex-wrap">
                        ...
                        <button
                          ...
                          className={`px-2 py-0.5 text-[11px] rounded-full border transition-colors ${
                            isExplicitlySelected
                              ...
                              : 'border-gray-200 dark:border-gray-600 text-qd-text-dim hover:bg-qd-panel-deep dark:hover:bg-qd-panel'
                          }`}
                        >
