...
export function Sidebar() {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      try {
        ...
        // Load agents in background (don't block initial load)
        fetchAgents();
      } catch (err) {
  ...
  return (
    <aside className="w-72 bg-slate-800 text-white flex flex-col overflow-y-auto">
      ...
      <div className="sticky top-0 bg-slate-800 p-4 border-b border-slate-700 z-10">
        ...
        <button
          ...
          className="w-full mt-2 flex items-center justify-center gap-2 px-4 py-2 rounded-lg transition-colors bg-slate-700 hover:bg-slate-600 text-slate-200"
        >
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {/* AGENTS Section */}
      <div className="px-3 pt-3 pb-1 border-b border-slate-700">
        ...
        >
          ...
          Library
          {agents.length > 0 && (
            <span className="ml-auto text-xs text-slate-400">{agents.length}</span>
          )}
        </button>
        <button
          onClick={() => {
      ...
      <div className="sticky bottom-0 p-4 border-t border-slate-700 bg-slate-800">
        <button
        ...
        >
          ...
          <div className="flex-1 text-left">
            ...
            <div className="text-xs text-slate-400">{currentModelName}</div>
          </div>
