...
export function Header({ 
  ...
  return (
    <header className="h-14 border-b border-white/30 bg-white/60 backdrop-blur-xl shadow-sm flex items-center px-6">
      <div className="flex items-center gap-3 flex-1 min-w-0">
        {sessionName ? (
          <>
            ...
            {isEditingName ? (
              <input
                ...
                className="font-medium text-gray-900 px-2 py-0.5 border border-blue-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500 w-[200px]"
                autoFocus
              />
            ) : (
              <h2 
                className={`font-medium text-gray-900 w-[200px] truncate ${onNameChange ? 'cursor-pointer hover:text-blue-600' : ''}`}
              ...
              <div className="relative">
                <button
                  ...
                  className={`min-w-[80px] h-[30px] px-2.5 py-1 text-xs font-medium rounded-md flex items-center gap-1.5 transition-colors duration-150 ${
                    isNewSession 
                      ? 'bg-violet-50/80 text-violet-700 hover:bg-violet-100 cursor-pointer' 
                ...
                {/* Model dropdown */}
                {showModelDropdown && (
                  <div className="absolute top-full left-0 mt-1 bg-white/80 backdrop-blur-xl border border-white/40 rounded-xl shadow-xl z-10 min-w-[150px] max-h-60 overflow-y-auto">
                    {availableModels.map((m) => (
                      <button
                        key={m.id}
                        onClick={() => handleModelSelect(m.id)}
                        className={`w-full text-left px-3 py-2 text-sm hover:bg-white/60 ${
                          m.id === model ? 'bg-violet-50/80 text-violet-700' : 'text-gray-700'
                        }`}
                      >
                        {m.name}
                      </button>
            ...
            {isEditingCwd && (
              <div className="flex items-center gap-1">
                <input
                  ...
                  className={`h-[30px] px-2.5 py-1 text-xs font-medium border rounded-md focus:outline-none focus:ring-1 w-64 ${
                    cwdError ? 'border-red-300 focus:ring-red-500' : 'border-blue-300 focus:ring-blue-500'
                  }`}
                  autoFocus
                ...
                <button
                  ...
                  className="h-[30px] px-1.5 py-1 text-xs bg-blue-50 text-blue-700 rounded-md hover:bg-blue-100 transition-colors duration-150 flex items-center"
            ...
            {cwdError && (
              <span className="text-xs text-red-600">{cwdError}</span>
            )}
          ...
          <h2 className="text-gray-500">Select or create a session</h2>
        )}
      </div>
