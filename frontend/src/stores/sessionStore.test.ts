...
describe('sessionStore', () => {
  ...
  describe('updateNewSessionSettings', () => {
    it('merges partial settings into existing settings', () => {
      useSessionStore.setState({
        newSessionSettings: { name: 'old', model: 'gpt-4.1', cwd: '/', mcpServers: [], tools: { custom: [], builtin: [], excluded_builtin: [] } },
      });
  ...
  describe('clearNewSession', () => {
    it('clears new session mode and settings', () => {
      useSessionStore.setState({
        isNewSession: true,
        newSessionSettings: { name: 'x', model: 'm', cwd: '/', mcpServers: [], tools: { custom: [], builtin: [], excluded_builtin: [] } },
      });
