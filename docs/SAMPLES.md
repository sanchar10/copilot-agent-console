...
Copilot Agent Console ships with pre-built agents, workflows, automations, and tools to get you started. These are copied to `~/.copilot-agent-console/` on first run.
...
Changes are preserved across upgrades — seed content only copies if the file doesn't already exist.
