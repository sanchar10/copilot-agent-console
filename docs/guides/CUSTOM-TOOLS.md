...
**Example:** _"Build a tool that gets the current weather for a city"_ → Tool Builder creates `free_weather.py` in `~/.copilot-console/tools/`, tests the import, calls the handler with sample data, and confirms everything works. Then ask any agent: _"What's the weather in Seattle?"_ and it will use your new tool.

## Managing Tools

Tools are stored in `~/.copilot-console/tools/`. Each `.py` file in this folder becomes available for selection in new sessions.
...
Each `.py` file in `~/.copilot-console/tools/` needs a `TOOL_SPECS` list defining the tool schema and handler:

```python
# ~/.copilot-console/tools/weather.py
