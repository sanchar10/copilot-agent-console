...
Patch release with three improvements: shipped sample workflows/agents/tools/automations are now **app-managed** and refreshed on every upgrade so future bug fixes actually reach existing installs (this fixes the silent-stale-seed class of bug that hid the v0.8.5 PowerFx `==` → `=` syntax fix from anyone who installed before v0.8.5); PowerFx evaluation errors now include the offending expression text instead of opaque "Error 12-13" position references; and the `playwright` MCP server is now seeded into `mcp-config.json` automatically on first launch (and on upgrade) instead of relying on the install scripts' bespoke JSON-mutation logic, removing ~55 lines of duplicated setup code per install script.

### 🐛 Fixes

- **Foreach loop variable in seed `workflow-feature-advanced.yaml`** — line 69 used `itemVariable: Local.person`, which the AF declarative SDK silently ignores (it accepts `iteratorVariable`, `item`, or `itemName`, falling back to the default `Local.item` for anything else). Result: the greet step printed `Hello, !` instead of `Hello, alice/bob/carol!`. Renamed to `iteratorVariable: Local.person`. Verified end-to-end on a Python 3.13 + powerfx machine.
- **PowerFx errors now name the failing expression _and_ the step that owns it** — `app/services/workflow_engine.py` installs a wrap around `agent_framework_declarative._workflows._declarative_base.WorkflowState.eval` at import time. When the SDK raises `ValueError` containing "Power Fx" (compile/runtime failure), the wrap re-raises with a step hint and the original expression text prepended: `PowerFx evaluation failed in step "branch_quick_eval" (kind=If) for expression \`=Local.mode == "quick"\`: Power Fx failed compilation: …`. The step hint is derived by walking `sys._getframe()` upward to find the active `DeclarativeActionExecutor` (display name + kind). Generic "For example, you can add the operand…" boilerplate from PowerFx is stripped, and `; ` separators are converted to bullet lines so multi-error compilation failures render readably in the Toast (which already honours `whitespace-pre-line`). Surfaces through the existing `workflow_failed` event, so both the toast and the inline workflow_failed red card show which expression and which step actually broke. Idempotent (re-import safe via `_copilot_console_pfx_wrap` flag), no-op when AF declarative isn't installed.
- **Inline workflow_failed red card honours newlines** — `frontend/src/components/workflows/WorkflowRunView.tsx` previously rendered the failure message as a single paragraph; added `whitespace-pre-line` so the bulleted multi-error list (from the PowerFx wrap) appears one error per line in the inline card just like in the Toast.
- **HumanInput Send no longer accepts empty input** — `FreeTextSubmit` in `WorkflowRunView.tsx` now disables the Send button (and ignores form submit) when the textbox value is empty or whitespace-only. Choice buttons remain unaffected.

### ✨ Improvements

- **Seed sync flipped to overwrite-by-default** — `app/services/seed_service.py:210` now passes `overwrite=True` for the `seed/copilot-console/ → ~/.copilot-console/` sync (previously `False`). Shipped samples (workflows, agents, tools, automations, docs, local-mcp-servers, etc.) are app-managed and refreshed on every version change so users automatically pick up bug fixes and new defaults — exactly the mechanism that v0.8.5's PowerFx `==` → `=` fix needed but didn't have. Users who want to customize a sample should copy it to a new filename (e.g. `workflow-feature-advanced.yaml` → `workflow-my-feature.yaml`); files with names not in the seed tree are never touched. The previously-separate "docs always overwrite" branch is now redundant and removed. `mcp-config.json` remains the single exception — it merges via `_merge_mcp_config` so user-added MCP servers are preserved.
- **Playwright MCP seeded by the app, not by install scripts** — `seed/copilot-console/mcp-config.json.template` now includes the `playwright` MCP server alongside `weather-server`. The existing `_merge_mcp_config` logic adds it on first launch (and on each app upgrade) for *every* install path — `pip`, `uvx`, scripted, dev — without overwriting user-added servers. The hand-rolled JSON-mutation code in `scripts/install.ps1` (~55 lines) and `scripts/install.sh` (~55 lines) is removed; both scripts now just carry a comment block explaining where Playwright comes from. Single source of truth, no more divergence between install-script and seed paths. Requires Node.js (`npx`) on PATH for use; the package is fetched on first invocation.
- **`docs/guides/INSTALL.md` Playwright section rewritten** — was a manual "add this JSON to your file" recipe; now documents that the server ships preconfigured, what's required to use it (Node.js), and how to disable it across upgrades (toggle off in Settings → MCP Servers, since deleting the entry would just be re-added on next version bump by the seed merge).
- **Toast UX overhaul** — `frontend/src/components/common/Toast.tsx` restructured into a windowed layout: a draggable title bar with the type label (INFO / SUCCESS / WARNING / ERROR), icon, and action/close buttons sits above a thin separator; the message body sits below as freely selectable text. Title bar gets a darker shade via `bg-black/20` overlay so it visually anchors. Drag-to-move uses pointer capture, lets users push a toast aside to read controls behind it without dismissing it. Text selection now works in the body — previous design used `select-none` everywhere to enable whole-toast drag, which blocked copy.
- **Workflow YAML editor: Tab indents instead of stealing focus** — `frontend/src/components/workflows/WorkflowEditor.tsx` adds an `onKeyDown` handler that intercepts Tab to insert two spaces (YAML convention) and Shift+Tab to outdent. Multi-line selection indents/outdents the whole block. No new dependencies; trade-off is that Tab no longer escapes the textarea — use Esc + Tab for keyboard navigation.
- **Mobile docs reframed alongside `/remote`** — README mobile bullet + section, `docs/index.html` Feature 2 (eyebrow / h2 / body), and `docs/guides/MOBILE-COMPANION.md` rewritten to position Mobile Companion as complementary to GitHub CLI's built-in `/remote`. Mobile Companion's value: every CLI session (Console-launched and native terminal) auto-discovered on your phone, plus push notifications when any agent finishes or asks a question. Removed inaccurate marketing claims about mobile-only features that don't exist (no workflow UI, cross-session search, agent library, or pinned-notes view on mobile — only Sessions list, Chat, Active-agents monitor, Settings).

### 🧹 Chores

- **Test collection unbroken** — added empty `tests/__init__.py`. Four test files (`test_concurrency_primitives.py`, `test_session_service_reasoning_effort.py`, `test_storage_service.py`, and one test in `test_session_config.py`) had been failing to even collect since commit `d4d8cd8` (Stage 2 backend restructuring) because they do `from tests.test_services import …` and `tests/` wasn't a proper package. Adding `__init__.py` resolves the import; full test count goes from 621 reachable → 657 reachable, all passing.
